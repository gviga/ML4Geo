...
checkpoint = torch.load(outf + '/../pointnet/basis_model_best_20.pth')
...
#checkpoint = torch.load(outf + '/desc_model_best_var.pth')
#classifier.load_state_dict(checkpoint)
classifier.cuda()

best_eval_loss=np.inf;
#best_eval_loss = 1.765246189199388;
...
for epoch in range(0,400):
    ...
    if eval_loss <  best_eval_loss:
        ...
        torch.save(classifier.state_dict(), '%s/desc_model_best_var.pth' % (outf))
        torch.save(basis.state_dict(), '%s/basis_model_best_var.pth' % (outf))

    train_losses.append(train_loss)
    eval_losses.append(eval_loss)

    np.save(outf+'/train_losses_desc_var.npy',train_losses)
    np.save(outf+'/eval_losses_desc_var.npy',eval_losses)
