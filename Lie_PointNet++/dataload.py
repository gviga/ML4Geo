...
#carica i dati dal set di surreal12k
class AnimalDataLoader(Dataset):
    def __init__(self, root,  npoint=1024, split='train', uniform=False, normal_channel=True, cache_size=15000, augm = False):
        ...
        #splitta training set e test set
        if split =="train":
            self.data = np.load("./data/animal_train.npy").astype(dtype=np.float32)
            #self.data = self.data
        if split =="test":
            self.data = np.load("./data/animal_test.npy").astype(dtype=np.float32)
            #self.data = self.data
        #valuation set
...
class HyppoDataLoader(Dataset):
    def __init__(self, root,  npoint=1024, split='train', uniform=False, normal_channel=True, cache_size=15000, augm = False):
        ...
        #splitta training set e test set
        if split =="train":
            self.data = np.load(os.path.join(root, "hyppo_train.npy")).astype(dtype=np.float32)
            #self.data = self.data
        if split =="test":
            self.data = np.load(os.path.join(root, "hyppo_test.npy")).astype(dtype=np.float32)
            #self.data = self.data
        EDGES_PATH = os.path.join(root,"hyppo_template.ply")
